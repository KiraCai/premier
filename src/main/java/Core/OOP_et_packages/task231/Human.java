package src.main.java.Core.OOP_et_packages.task231;

/*Использовать различные модификаторы доступа
...
3. experience — опыт. Каждый человек обладает некоторым жизненным опытом,
делиться с которыми он хотел бы либо с детьми, либо с теми, с кем находится в одном доме (пакете)
...
public class Human {
    ...
    public static void main(String[] args) {
        Human person = new Human("number1", "news1", "exp1", "goss1");
        System.out.println(person.getSecret());
        System.out.println(person.getNews());
        System.out.println(person.getExperience());
        System.out.println(person.getGossip());
    }
}
